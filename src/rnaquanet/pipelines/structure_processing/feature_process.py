...
from rnapolis import parser, tertiary
...
from rnaquanet.pipelines.structure_processing.feature_compression import (
                ...
                if i in paired_with_dict.keys():
                    for residue in paired_with_dict[i]:
                        try:
                            ...
                            logging.info(
                                f"Error in {filtered_pdb_file_path} {i} {residue[0]} {residue[1]} {residue[2]} "+str(e))
                for bp_t in PAIRINGS_FEATURES:
                    pairings[bp_t].append(residue_pairings[bp_t])
...
def clear_na_features(df: pd.DataFrame, features_preferences) -> pd.DataFrame:
    ...
    match features_preferences["na_solution"]:
        case "ENCODER":
            return pd.concat(
                [
                    compress_cytosine_features(df),
                    compress_uracil_features(df),
                    compress_guanine_features(df),
                    compress_adeine_features(df),
                ],
                axis=0,
            )
        case "NONE":
        ...
        case "ANTCZAK":
            raise NotImplementedError("Antczak's method is not implemented yet")
